...
class Batch {
    static hget(key, field, client = 'default') {
        ...
        if (!batchs.get(_key)) batchs.set(_key, new DataLoader(keys => {
            return new Promise(resolve => {
                Redis.getClient(client).hmget(key, keys, (error, values) => {
                    resolve(values || [null]);
                });
            });
        }, {
    ...
    static hset(key, field, value, client = 'default') {
        ...
        if (!batchs.get(_key)) batchs.set(_key, new DataLoader(keys => {
            ...
            return new Promise(resolve => {
                Redis.getClient(client).hmset(key, values, () => resolve(keys));
            });
        }, {
            cache: false
        }));
    ...
    static sismember(key, value, client = 'default') {
        ...
        if (!batchs.get(_key)) batchs.set(_key, new DataLoader(keys => new Promise(resolve => {
            Redis.getClient(client).multi(keys.map(value => ['sismember', key, value])).exec((error, replies) => resolve(replies));
        }), {
            cache: false
        }));
...
['hdel', 'sadd', 'srem', 'lpush', 'rpush'].forEach(command => {
    Batch[command] = function (key, value, client = 'default') {
        ...
        if (!batchs.get(_key)) batchs.set(_key, new DataLoader(keys => new Promise(resolve => {
            Redis.getClient(client)[command](key, keys, () => resolve(keys));
        }), {
            cache: false
        }));
...
['spop', 'srandmember'].forEach(command => {
    Batch[command] = function (key, client = 'default') {
        ...
        if (!batchs.get(_key)) batchs.set(_key, new DataLoader(keys => new Promise(resolve => {
            Redis.getClient(client)[command](key, keys.length, (error, values = []) => resolve(new Array(keys.length).fill(null).map((value, i) => values[i])));
        }), {
            cache: false
        }));
